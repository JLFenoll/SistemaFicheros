...
public class NFController {
	 ...
	 */
	private static final byte SERVER_IN = 2;
	...
	public void processCommand() {
		...
		case NFCommands.COM_FGSERVE:
			 ...
			 */
			controllerDir.registerFileServer(10000);
		...
		case NFCommands.COM_STOP_SERVER:
			 ...
			 * 
			 * MODIFICACION: **EN CASO DE QUE NO SE PUEDA DAR DE BAJA COMO SERVIDOR DE FICHEROS
			 * 				 SEGUIRÁ SIENDO UN SERVIDOR DE FICHEROS Y SE IMPRIMIRÁ POR PANTALLA
			 * 				 UN MENSAJE QUE LO EXPLIQUE**
			 */
			...
			if(commandSucceeded == true) {
				...
				System.out.println("* You are still server, because unregister server failed");
			}
			break;
	...
	public boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_LOGOUT:
			...
			}else if(currentState == SERVER_IN) {
				commandAllowed = false;
				System.err.println("* You cannot logout because you are serving files, you must stop serving to logout");
			}
			break;
		case NFCommands.COM_USERLIST:
			if (currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot get the userList because you are not logged in from the directory");
			}
			break;
		case NFCommands.COM_FGSERVE:
			if(currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot be a peerServer because you are not logged in from the directory");
			}else if(currentState == SERVER_IN) {
				commandAllowed = false;
				System.err.println("* You cannot be a peerServer because you are already serving");
			}
			break;
		case NFCommands.COM_DOWNLOADFROM:
			if(currentState == LOGGED_OUT) {
				commandAllowed = false;
				System.err.println("* You cannot download because you are logged in from the directory");
			}
			break;
		case NFCommands.COM_BGSERVE:
			if(currentState == LOGGED_OUT) {
		...
		case NFCommands.COM_STOP_SERVER:
			if(currentState == LOGGED_OUT) { 
				commandAllowed = false;
				System.err.println("* You cannot stop being a peerServer because you are not logged in from the directory");
			}else if(currentState == LOGGED_IN) {
				commandAllowed = false;
				System.err.println("* You cannot stop being a peerServer because you are not serving files");
			}
			break;
		default:
