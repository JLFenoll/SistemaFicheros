...
public class NFControllerLogicDir {
	...
	protected boolean getAndPrintUserList() {
		 ...
		 */
		boolean result = false;
		if(directoryConnector.getUserList() != null) {
			result = true;
		}
	...
	/*
	 * Método que comprueba que la dirección proporcionada esta en uso o no
	 * 
	 * @param serverNicknameOrSocketAddr El nick o IP:puerto del servidor por el que
	 *                           		preguntamos
	 * @return Devuleve true si la dirección está en uso y false en caso contario
	 */
	public static boolean isAddressInUse(InetSocketAddress address) {
	    try {
	    	Socket socket = new Socket();
	    	socket.connect(address, 1000); // Intenta conectar con un timeout de 1 segundo
	    	socket.close(); // Cierra el socket si la conexión fue exitosa
	    	return true; // Si no se lanzó una excepción, entonces la dirección está en uso
	    } catch (Exception e) {
	        return false; // Si se lanzó una excepción, la dirección no está en uso
	    }
	}
	...
	public InetSocketAddress getServerAddress(String serverNicknameOrSocketAddr) {
		...
		if (serverNicknameOrSocketAddr.contains(":")) { // Then it has to be a socket address (IP:port)
			 ...
			 */
			try {
				int indiceSeparador = serverNicknameOrSocketAddr.indexOf(":");
				String IP = serverNicknameOrSocketAddr.substring(0, indiceSeparador);
				InetAddress serverIP = InetAddress.getByName(IP);
				String PORT = serverNicknameOrSocketAddr.substring(indiceSeparador + 1);
				int puerto = Integer.parseInt(PORT);
				fserverAddr = new InetSocketAddress(serverIP, puerto);
				if(!isAddressInUse(fserverAddr)) {
					fserverAddr = null;
					System.out.println("This address is not in use, check the port");
				}
			} catch (Exception e) {
				System.out.println("Invalid port, must be in [0-65536]");				
			}
